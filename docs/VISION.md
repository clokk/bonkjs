...
A 2D game engine built for AI collaboration. MDX scenes that humans and LLMs can read, write, and iterate on together. Opinionated defaults, cross-platform export from day one.
...
3. **MDX scenes compile to data, not React components** - Game loop owns rendering.
...
3. **MDX scenes are readable** - AI can understand existing scene structure
...
2. Claude: Generates scene.mdx + PlayerController.ts
...
| Scene format | Binary/YAML | **MDX** | AI-readable, version control friendly |
| Inspector serialization | C# attributes | **TypeScript types** | AI can read/write, no decorators needed |
| Prefabs | Binary asset | **MDX snippet or .prefab.mdx** | Readable templates |
...
The MDX scene can set these directly:
```mdx
<Behavior src="./PlayerController.ts" props={{ speed: 8, health: 150 }} />
```
...
| Scene Format | MDX → JSON | AI-readable source, data at runtime |
